...
///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  start@{
//orig:static void safe_print(const char *cp, int len)
static void safe_print(const char *cp, int len, int has_arg_x)
{
	unsigned char	ch;
	...
	/* modify for SDcard's label shows incorrectly, begin */
#if 0
	while (len--) {
		ch = *cp++;
		if (ch > 128) {
...
#else
	int size = len*4+128;
	char *label1 = (char*)malloc(sizeof(char)*size);
	char *label2 = (char*)malloc(sizeof(char)*size);
	memset(label1, 0, size);
	memset(label2, 0, size);
	int pos1 = 0, pos2 = 0;
	int err = 0;
	...
	free(label2);
#endif
///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  end@}
}
...
///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  start@{
//origin:static void print_tags(blkid_dev dev, char *show[], int numtag, int output)
static void print_tags(blkid_dev dev, char *show[], int numtag, int output, int has_arg_x)
///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  end@}
{
	blkid_tag_iterate	iter;
	...
	while (blkid_tag_next(iter, &type, &value) == 0) {
		...
		if (output & OUTPUT_VALUE_ONLY) {
			...
			fputs("=\"", stdout);
            ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  start@{
			//origin:safe_print(value, -1);
	        safe_print(value, -1, has_arg_x);
            ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711  end@}
			fputs("\" ", stdout);
		}
	}
...
int main(int argc, char **argv)
{
	...
	int lookup = 0, gc = 0;
	int c;
	int has_arg_x = 0;///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711

    ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711 start@{
    /*orig
	while ((c = getopt (argc, argv, "c:f:ghlLo:s:t:w:v")) != EOF)
    */
	while ((c = getopt (argc, argv, "c:f:ghlLo:s:t:w:v:x")) != EOF)
    ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711 end@}
		switch (c) {
		case 'c':
			read = optarg;
	...
	if (lookup) {
		...
		if ((dev = blkid_find_dev_with_tag(cache, search_type,
						   search_value))) {
            ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711 start@{
			//origin:print_tags(dev, show, numtag, output_format);
            print_tags(dev, show, numtag, output_format, has_arg_x);
            ///FW: added by liucy modify for SDcard's label shows incorrectly LENOVOGSSIS-217 from YOGAX 20210711 end@}
			err = 0;
		}
